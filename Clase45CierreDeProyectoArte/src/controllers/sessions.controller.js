import { galleryService, userService } from "../services/services.js";
...
const register = async(req,res) =>{
    ...
    const gallery = await galleryService.save()
...
const login = async(req,res) =>{
    ...
    if(email===config.app.ADMIN_EMAIL&&password===config.app.ADMIN_PASSWORD){
        const tokenizedAdmin = UserDTO.getTokenFrom({role:'admin'});
        const token = jwt.sign(tokenizedAdmin,config.jwt.SECRET,{expiresIn:"1d"});
        return res.cookie(config.jwt.COOKIE,token).send({status:"success",message:"Logged in"})
    }
    ...
    const tokenizedUser = UserDTO.getTokenFrom(user);
