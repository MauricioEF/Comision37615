...
const addArtwork = async(req,res) =>{
    ...
    const userGallery = await galleryService.getBy({_id:user.gallery});
    console.log(userGallery);
    userGallery.artworks.push({artwork:artworkId})
    await galleryService.update(userGallery);
    res.send({status:"success",message:"Artwork added to users gallery"})
}
